...
# #######################################################################
               ...
               SRCS = xpuz.cpp objects.cpp stack.cpp imgbuff.cpp puzzle.cpp                       real.cpp vec2.cpp vec2list.cpp mat2.cpp                       color_mapper.cpp base_image.cpp gif_image.cpp ximage.cpp                       jpeg_image.cpp

           CXXFLAGS = $(USE_MIT_SHM) $(RANG_CHECK)

    LOCAL_LIBRARIES = $(XLIB)  -Wl,-Bstatic -ljpeg -Wl,-Bdynamic -lm

               OBJS = xpuz.o objects.o stack.o imgbuff.o puzzle.o                       real.o vec2.o vec2list.o mat2.o                       color_mapper.o base_image.o gif_image.o ximage.o                       jpeg_image.o
...
install:: xpuz
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  xpuz $(DESTDIR)$(BINDIR)/xpuz

all:: xpuz.$(MANNEWSUFFIX)

xpuz.$(MANNEWSUFFIX): xpuz.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname xpuz` && \
	   $(LN) `basename xpuz.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < xpuz.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) xpuz.$(MANNEWSUFFIX)

install.man:: xpuz.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) xpuz.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/xpuz.$(MANSUFFIX)
